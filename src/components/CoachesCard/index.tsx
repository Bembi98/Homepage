...
class StudentCard extends React.Component<Props, State> {
    render() {
        const {student, classes, onDelete,handleSelectCourse,handleOpenEditModal} = this.props;
        return (
            <Card variant="outlined" className={classes.card}>
                <CardMedia className={classes.cardImg}>
                    <img className={classes.img} src={student.img} alt={student.name}/>
                    <Button className={classes.btnCourse} variant="outlined" onClick={handleSelectCourse}> {student.info} </Button>
                </CardMedia>
